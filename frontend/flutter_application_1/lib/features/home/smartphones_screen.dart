...
class _SmartphonesScreenState extends State<SmartphonesScreen> {
  ...
  Future<List<ProductModel>> _loadProducts() {
    return _productService.fetchCollectionProducts(collection: 'phones', limit: 40);
  }
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return const loading_widgets.LoadingWidget(
                        message: 'Loading phones from Firestore...',
                      );
                    }

                    if (snapshot.hasError) {
                      return loading_widgets.ErrorWidget(
                        message: 'Failed to load phones',
                        onRetry: _refresh,
                      );
                    }
                    ...
                    if (products.isEmpty) {
                      return const loading_widgets.EmptyStateWidget(
                        ...
                        subtitle: 'Your Firestore phones collection is empty.',
                      );
                    }
